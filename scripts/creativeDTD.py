...
class creativeDTD:
    ...
    def __init__(self, args, data_path: str, model_path: str):
        ...
        ## load datasets
        self.entity_embeddings_dict = utils.load_graphsage_unsupervised_embeddings(self.args.data_dir)
        self.args.entity2id, self.args.id2entity = utils.load_index(os.path.join(self.args.data_dir, 'entity2freq.txt'))
        self.args.relation2id, self.args.id2relation = utils.load_index(os.path.join(self.args.data_dir, 'relation2freq.txt'))
        ...
        with open(os.path.join(args.data_dir, 'filtered_drug_nodes_for_precomputation.pkl'), 'rb') as f:
            drug_curie_ids = pickle.load(f)
        self.drug_curie_ids = drug_curie_ids
    ...
    def set_query_disease(self, disease_curie: str):
        bool_value, normalized_disease_curie = utils.check_curie_available(logger = self.args.logger, curie = disease_curie, available_curies_dict = self.args.entity2id)
        if bool_value:
