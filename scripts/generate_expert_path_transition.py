"""Generate expert path transition states for pre-training the AC model."""
...
def generate_expert_path_transition(args):
    ...
    with open(pretrain_path_file, 'rb') as f:
        pretrain_path = pickle.load(f)
    ...
    while batch_path[0].shape[1] < (args.max_path + 1):
    ...
    with open(expert_trans_file, 'wb') as f:
        pickle.dump(expert_transitions, f)
    args.logger.info(f"Generated expert transitions to {expert_trans_file}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_name", type=str, help="log file name", default="step18_generate_expert_path_transition.log")
    ...
    generate_expert_path_transition(args)
