...
if __name__ == "__main__":
	...
	## Other parameters
	parser.add_argument('--threshold', type=float, help='threshold to filter drug predictions', default=0.6)
	...
	main_args.pretrain_model_path = os.path.join(main_args.model_path,'RF_model_3class','RF_model.pt')
	...
	## create xDTD object
	dtd = creativeDTD(main_args, data_path, model_path)

	## get disease info
	disease_set = pd.read_csv(main_args.disease_set, sep='\t', header=0)
	disease_list = list(disease_set['id'])
	existing_disease_list = [os.path.splitext(file_name)[0].replace(f'top{main_args.N_drugs}drugs_','') for file_name in os.listdir(os.path.join(output_path, 'prediction_scores'))]
