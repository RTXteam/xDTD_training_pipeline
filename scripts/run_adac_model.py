"""Train the ADAC RL model (modified from Zhao et al. doi: 10.1145/3397271.3401171)."""
import argparse
import math
import os
import pickle
...
def train(args):
    ...
    expert_transitions_source_ids = expert_transitions[0].state[:, 0].numpy()
    ...
    train_df = pl.read_csv(os.path.join(args.data_dir, 'RL_model_train_val_test_data', 'train_pairs.txt'), separator='\t')
    cols = train_df.columns
    train_pairs = np.column_stack([
        [kg.entity2id[x] for x in train_df[cols[0]].to_list()],
        [kg.entity2id[x] for x in train_df[cols[1]].to_list()],
    ])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while train_dataloader.has_next():
            ...
            expert_trans_curr_idx = np.where(np.isin(expert_transitions_source_ids, list(source_set)))[0].tolist()
        ...
        if epoch > args.ac_update_delay:
            ...
            os.makedirs(policy_model_save_location, exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_name", type=str, help="log file name", default="step20_run_adac_model.log")
    ...
    args.model_save_location = os.path.join(args.output_folder, 'ADAC_model')
